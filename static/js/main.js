...
document.querySelectorAll("img").forEach((image) => {
  ...
  // Keep wheel, touchpad, and keyboard scrolling native so the page responds
  // during input instead of easing toward an accumulated target afterward.
...
(function () {
  ...
  function buildCards() {
    ...
    cards.forEach((item, index) => {
      ...
      if (item.type === 'image') {
        card.innerHTML = `
          <img src="${item.src}" alt="${item.alt || item.title}" loading="eager" decoding="async" fetchpriority="low" draggable="false" />
        `;
      } else {
        card.innerHTML = `
